...
Konfig::Konfig(char* nev ,long double ertek)
{
  first = new Konfitem(nev,ertek);
  last=first;
}
...
void Konfig::Newitem(char* nev,long double ert)
{
 last->Setnext(new Konfitem(nev,ert));
 last=last->Getnext();
}
...
void Konfig::Set(char* nev,long double ertek)
{ Konfitem * mutato=exists(nev); 
 if (mutato==NULL) 
 {  
    strncpy(tempb,nev,K_BHOSSZ);
...
// ellenoriz(patt,nevv,szam) i=0; char nevv[]=patt; \
//   while ( nevv[i]==nev[i] && nevv[i]!='\0' && nev[i]!='\0' ) i++;    \
//   if (nevv[i]=='\0' && nev[i]=='\0' && i != 0) return (szam);
...
Konfitem::Konfitem(char* nev,long double ert)
{  
  Setname(nev);
  Setval(ert);
  next=NULL;
...
void Konfitem::Setval(long double szam)
{ 
  snprintf(ertek,K_BHOSSZ,"%Lg",szam);
}
